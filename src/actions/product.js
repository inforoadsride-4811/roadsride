...
export async function getProductBySlug(slug) {
  try {
    ...
    const transformed = {
      ...
      images: product.images ? product.images.map(img => ({ id: img.id, src: img.src, alt: img.alt })) : [],
      ...
      packs: product.variants.map((v) => ({
        ...
        images: (v.images && v.images.length > 0) ? v.images : (product.images ? product.images.map((img) => ({ id: img.id, src: img.src, alt: img.alt })) : []),
      })),
      ...
      reviews: product.reviews.map((r) => ({
        ...
        adminReplyAt: r.adminReplyAt ? new Date(r.adminReplyAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : null,
